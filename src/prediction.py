...
# only fake real prediction without attribution prediction
# # prediction.py
# import torch
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # ----- Most likely issues (with conditions) -----
        print("\n✨ Most likely issues:")
        is_fake = "Fake" in label_str  # since label_str is "Real News ✅" or "Fake News ❌"
        ...
        if is_fake:
            ...
            found_any = False
            for i in sorted_indices:
                if fg_probs[i] > 0.5:  # keep only issues above 0.5
                    found_any = True
                    print(f"   {FG_CLASSES[i]} → {fg_probs[i]:.4f} ({fg_probs[i] * 100:.2f}%)")

            if not found_any:
                print("   No fine-grained issues above 0.5 probability.")
        else:
            # For real news, don't list issues
            print("   No considerable issues found for this article.")
